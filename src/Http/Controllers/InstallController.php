...
namespace SmartLearnPro\SlpInstaller\Http\Controllers;
...
use SmartLearnPro\SlpInstaller\Events\EnvironmentSaved;
use Illuminate\Support\Facades\Http;
use Illuminate\Support\Facades\Log;
use SmartLearnPro\SlpInstaller\Http\Helpers\DatabaseManager;
...
class InstallController extends Controller
{
    ...
    public function preInstall()
    {
        ...
        return view('smartlearnpro.installer.pre-install', compact('route_value', 'resource_value', 'public_value', 'storage_value', 'env_value'));
    }
    ...
    public function configuration()
    {
        ...
        if (session()->has('validated')) {
            $response = Http::acceptJson()->post('https://google.com'.config('app.app_code'));
            try{
            ...
            return view('smartLearnpro.installer.config', $data);
        }
        return redirect(route('SlpInstaller::pre-install'));
    }
    ...
    public function serverValidation(Request $request)
    {
        ...
        if($this->phpversion() > 7.0 && $this->mysqli() == 1 && $this->curl_version() == 1 && in_array($this->allow_url_fopen(), ['1','on','On', 1]) == 1 && $this->openssl() == 1  && $this->zip() == 1 && $this->pdo() == 1 && $this->bcmath() == 1 && $this->ctype() == 1 && $this->fileinfo() == 1 && $this->mbstring() == 1 && $this->tokenizer() == 1 && $this->xml() == 1 && $this->json() == 1){
            ...
            return redirect(route('SlpInstaller::config'));
        }
    ...
    public function final(Request $request)
    {
        ...
        if (config('app.app_code')) {
            ...
            if (!$isValidDomain) {
                ...
                $response = Http::acceptJson()->post('https://google.com', $requestData);
                ...
                if ($response->successful()) {
                    ...
                    if ($data->status === 'success') {
                        ...
                        return Redirect::route('SlpInstaller::database');
                    } else {
